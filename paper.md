...
# Available Methods
The LCIA formatter is structured to easily convert original source data from existing LCIA methods. Version 1.0 of the LCIA formatter converts three commonly used LCIA methods: TRACI 2.1, ReCiPe2016, and ImpactWorld+ [@Tbl:methods]. Available LCIA methods cover a variety of impact categories, with midpoint and/or endpoint indicators available at different points on the environmental cause-effect chain [@unep_glam_2016]. Cumulative LCI indicators based on the FEDEFL are also available as an output of the LCIA formatter.
