...
The LCIA formatter is intended to provide an open solution to this problem, by transparently acquiring LCIA data from LCIA provider portals and mapping the elementary flows used in the characterization factors of those datasets (CF) to an authoritative set of elementary flows used in LCI data (E), to provide CFs transparently and harmonized with data objects used in E.

The LCIA formatter module this paper describes is a specific solution to this problem. The LCIA Formatter transparently acquires LCIA methods from original provider data portals, maps them to an authoritative flow list, and export them in common data formats for use with LCI datasets mapped to the same authoritative flow list. The LCIA formatter v1.0 uses the Federal LCA Commons Elementary Flow List (FEDEFL) as the authoritative system of elementary flows. This system has been adopted by federal agencies in the U.S. for use in sharing data through the Federal LCA Commons [(FLCAC)](https://www.lcacommons.gov/) [@edelen_federal_2019]. The automation of this process provided by the LCIAformatter is critical, because as elementary flows used in LCI are updated, the LCIA datasets should also be automatically updated, and vice versa, to facilitate their use in LCA. 
